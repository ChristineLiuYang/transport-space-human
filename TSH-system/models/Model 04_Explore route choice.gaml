		...
		loop aBuilding over: residential_building {
			...
			* */
			...
			create people number: 1 with: [location::any_location_in(aBuilding)] returns: List_of_Residents {
			    living_place <- aBuilding ;
		        working_place <- one_of(industrial_building) ;
		        write "working_place defined" + working_place.ID + "by agent" + id;
			    leisure_place <- one_of (leisure_building) ;
			}
			
			write "Residential building: " + aBuilding.ID + ", create Residents: " + length(List_of_Residents);
			loop aPeople over: List_of_Residents {
				write "This is resident: " + aPeople.getName(aPeople) + "with work place: " + aPeople.working_place.ID;
...
species people skills:[advanced_driving] {
	...
	reflex time_to_work when: current_date.hour = start_work and goal = "rest"{
		...
		if destination = nil{
			write "destination undefined for working place" + working_place.ID;
		}
		write "current time is: " + current_date.hour ;
	...
	*/
	...
	path route_choice (point aDestination) {
		node_close_to_destination <- roadNode closest_to(aDestination) ;
		if aDestination = nil{
			write "aDestination undefined" ;
		}
		if road_network = nil{
			write "road_network undefined" ;
		}
		if node_close_to_destination = nil{
			write "node_close_to_destination undefined" ;
		}

		path_followed <- compute_path(graph: road_network, target: node_close_to_destination); // the variables may be empty
		return path_followed ;

		write "Source node of the chosen route is: " + path_followed.source;
	}
	/* 
	action to_move {
		path_followed <- route_choice (destination) ;
